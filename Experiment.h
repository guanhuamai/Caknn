...
class Experiment{
private:
    string configID; // identification of the configuration
    ...
    Experiment(string configuration){
        ...
        configStream >> configID;  // generated by the slave script

        configStream >> projectPath;
        configStream >> dataName;
        configStream >> dbname;  dbname = configID + "." + dbname;
        configStream >> logname;  logname = configID + "." + logname;
    ...
    static void destructEverything(){
        ...
        SafeRegion::destructSafeRegion(); // 8 destruction
    }
...
public:
    ...
    static void display(){
        ...
        if (experiment == NULL) cout << "sorry we have nothing to tell..." << endl;
            ...
            cout << "meta config: " << experiment->metaConfig << endl;
            cout << "project path: " << experiment->projectPath << endl;
            cout << "data name: " << experiment->dataName << endl;
    ...
    static void buildExperiment(string configuration){
        destructExperiment();
        experiment = new Experiment(configuration);

        display(); // display while finish construction, convenient to check
    ...
    static void saveStatistics(){
        long long throughPut = Utility::getThroughPut(int(experiment->updateTime.count() * 1000),
                                                      experiment->updateCnt);

        ofstream logFileStream;
        logFileStream.open(experiment->logname.c_str(), ios_base::app);


        logFileStream << experiment->metaConfig;

        logFileStream << "\t" << experiment->expandCnt
